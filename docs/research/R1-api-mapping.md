...
- **响应 `data`**：结构同管理端 **9.3**（仅本人数据）。形状是**明确的** —— 见下方更正段 §7.3。来源：:1135。
...
- ~~**13.16 积分明细响应形状**：仅写「结构同管理端 9.3」，但管理端 9.3 字段未在本文档贴出 → C 端 TS 类型缺准确定义。`[未明确]`~~ **【2026-09-22 已更正，不是缺口】** 形状就在本文档 §9.3 的 JSON 块里（`:588-596`）。详见 §7.3。
...
## 8. 更正（2026-09-22，D4 #8 grilling 期间复核）

### 8.1 [`[未明确]` → 事实] 13.16 积分明细的响应形状**是明确的**，就在本文档 §9.3

原文把 §13.16 标为「`[未明确]` 文档未贴出字段形状」，并在 §6 记为「需联调确认」。**这是漏读** —— 用户端 `:1135` 写的是「结构同管理端 9.3」，而 §9.3（`:583` 起）**就在本文档里，并且贴了完整 JSON**：

| 出处 | 原文 |
|---|---|
| `API设计文档.md:583` | `### 9.3 积分明细 GET /api/v1/users/:id/score-logs` |
| `API设计文档.md:585-596` | 查询参数 `page`/`pageSize`；响应 `data = { list: [{ id, delta, reason, type, operatorName, createdAt }], total }`；`type`: `AUTO`（规则触发）/ `MANUAL`（人工调整） |
| `API设计文档.md:1135` | 「响应 `data`：结构同管理端 9.3（仅本人数据）」 |

**结论：C 端 `13.16` 的字段集 = `{ id, delta, reason, type, operatorName, createdAt }` + `total`，无需向后端确认。**

原文的建议（「向主 agent 要管理端 9.3 的准确字段」）也**不成立**：管理端字段同样在本文档 §9.3 里，不存在跨文档缺口。

**对下游的影响（已据此裁决）**：D4（`#8`）据此定了信用分明细页的版式（`04` §7.3 —— 展示 `delta`/`reason`/`createdAt` + `type` 映射为「系统/管理员」，**不展示 `operatorName`**），以及 `reason` 的脱敏判据（`type === "AUTO"` 与 `blacklistRecord.auto` 共用一条规则）。

### 8.2 复核方式

本节由 D4（`#8`）grilling 期间的主 agent 复核，**直接读 `java/smart-locker/docs/API设计文档.md` §9.3 与 §13.16 原文**，未使用二手转述。除本节外，本底稿其余结论（16 端点契约、四类时序、令牌判定式、15 条字段陷阱，以及 §7 的 13.6 鉴权更正）**全部维持有效**。
