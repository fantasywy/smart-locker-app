...
| 13.6 | 扫码查柜机 | `GET /api/app/v1/lockers/by-code?code=` | **无（§1 例外）**，但 §13 总则称需 access → `[冲突]` | 扫码/输入柜机 code 选柜 |
...
- **鉴权**：`[冲突]`。§13 总则（:917）称「除 13.1/13.2 外均需 `Authorization: Bearer`」；但 §1 路由例外清单（:25）明确把 `/api/app/v1/lockers/by-code` 列为免 access 的「按编号查询的动作型查询」。两处矛盾。**推荐实现按「免 access」处理（扫码选柜是入口，不应强制登录），但需主 agent 与后端确认哪条为准。** 来源：:917 与 :25。
...
### 已确认的文档内部冲突
1. **13.6 鉴权冲突**：§13 总则（:917）「除 13.1/13.2 外均需 Bearer」 vs §1 路由例外（:25）把 `/lockers/by-code` 列为免 access 查询。→ 需确认扫码查柜机是否强制登录。
