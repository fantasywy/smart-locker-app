...
//                               └─有─→ 带 access 发请求
//
// ⚠️ 本模块**只做分叉与「无 refresh」那条路**。有 refresh 时「带 access 发请求 →
// 撞 401 → 刷新 / 重登」由请求层内部消费（#19，见 `request/index.ts`），
// 启动闸门与全局失败出口属 #20。
// 「有 refresh」在这里的正确行为就是**什么都不做**（直接放行）—— 见 `ensureLoggedIn`。
//
// ⚠️ 它也是 #19「清态重登」那条路的**反面**：重登失败时 `clearTokens()` 已经清过态，
// 这里就**没有** refresh 了 —— 于是下次冷启动会走回「静默登录」而不是放行一个假登录态。
// 清态先于重登，正是为了这一点（见 `request/token.ts` 的 `clearTokens`）。
//
