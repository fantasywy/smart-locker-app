...
- **D4 resolved**（`#8`，2026-09-22）—— **受限态与准入类异常：黑名单与低信用分的降级体验**。三轮 grilling 收口（8 + 6 + 4 问，三次均按推荐）。**票面前提被修正**：`5004` **只封「开新动作」，不封「收尾动作」** —— 直接核 `13.11` 规则 1–4（`API设计文档.md:1057-1084`）**不含 `5004`**，`13.10` 取消与全部读端点也都不拦 ⇒ 准确表述是「**能看、能做完手上的、不能开新的**」，受限用户手上已存入的单照样能取出来。要点：① **承载模型 = 只在「我的」页常驻 + 动作点就地解释**，不挂全局横幅（后端意图本就是「能看不能做」，全局横幅会把一个还能正常用的 App 渲染成「你被封了」）；② 三个被拦动作（`13.7` / `13.12` / `13.15`）**一律保持可见、可点、点击后解释**，**不置灰不隐藏**（置灰不给原因只是裸错误码的变体）；③ **`5004` 特判，不走通用 `message` 通道**（服务端 message 既不解释原因也不给出路），其余错误码仍按 `message` 直出 —— 落点归 D7 请求层；④ **一律事后解释、绝不事前阻断** —— `13.1` 响应就带 `user.status`，但**拦截权只能属于服务端**，本地快照会造出双源真相、会把**已解禁用户挡在门外**，也不做本地 `status` 快照（承 `01` §3.2）；⑤ **存量单收尾权 = 取件开门（`13.11`）+ 取消（`13.10`），两者都保持可用且都不额外解释**，详情页取件区块只补一行「账号受限不影响取回本单」—— 解掉「我的东西拿不出来」这个最大恐慌，且不需要后端做任何事；⑥ **归因纪律：只陈述受限、不猜**（`5004` 遮蔽 `4004/8002/8003/6201/6202`，`13.15` 的 `5004` 判在 `8003` 之前），代价照实记：受限用户看到一个**被遮蔽过的世界**；⑦ **`reason` 两处共用一套分叉，判据 = `AUTO`/`auto`** —— 系统生成走**映射脱敏**、人工填写**直出原文**，定性为**展示层脱敏、不改写历史**；⑧ 申诉**说机制不说天数**（`releaseCondition` 只在管理端 9.4，C 端契约没有）；⑨ **客服只做 `open-type="contact"` 一个按钮** —— ⚠️ 它**不可运行时探测**（配置在微信后台，未配时点击静默无反应）⇒ **不做运行时降级、不做电话双出口**，「后台须配客服」为**上线前置项**，**顺带清掉 `01` §5 的「待复核」**；⑩ 信用分明细**独立成页**（`pageSize=20` + 上拉，行 = `delta` 正负号显式 + `reason` + `createdAt` + `type`，**不展示 `operatorName`**），**扣分用警示/危险档**（⚠️ 不套股票红涨绿跌）；⑪ **跨票硬规则：不得展示任何未经服务端回显的值** —— 判据是「服务端是否在响应里回显过」而非「是不是配置项」，`openTimeoutMinutes` 的唯一合法位置 = `03` §8 已定的 P1 建单等待态；⑫ **本票兜住 `4004` / `8002`**（与 `5004` 共享同一事务与校验序；经查 `03` §12 与 `01` §7 均未认领），只写失败表现、**不写恢复机制**；⑬ **不留痕、对称回落**（解除后 `status`→`NORMAL`、`blacklistRecord`→`null`，不做本地快照）。产物：**`docs/spec/04-blacklist-and-degradation.md`** + **`CONTEXT.md` 增补 3 条术语**（**信用分** `Avoid` 积分 —— 扣减型行为约束、非可兑换消费积分；**受限** `Avoid` 封号/被拉黑，「黑名单」保留为后端字段概念、**不进用户文案**；**单笔校验被拒** —— 与「受限」显式分开，尤其在屏端「业务拒绝」语境下）。**就地补正**：`docs/spec/01-entry-and-identity.md` §5（积分明细形态回填、客服载体定案、术语改「信用分」）+ §9 销位、`docs/spec/03-order-list.md` §12 销位（含 `03` §12 留问「是否影响下单入口」的答案：**不影响**）。**更正 R1 findings**（分支 `research/r1-api-mapping`，commit `19c8bea`）：`13.16` 标为 `[未明确]` 是**漏读** —— 字段形状就在 `API设计文档.md` §9.3（`:588-596`），新增 §8 更正段。**登记项**：C 端缺 `releaseCondition`；C 端无参数查询端点；上线前置项（配客服）。**本票边界**：只兜准入类，单笔业务校验失败（`3002`/`3001`/`6201`/`6202`/`8003`/`6001`）留在各自动作所在票。


## Not yet specified

预约占位窗口的用户表达；支付一期形态；技术栈与工程形态；视觉语言移植。（「取件码在小程序与柜机屏之间的流转」已于 2026-09-21 由 D2 graduate 成 `#6` 并 resolve；「订单首页对未存入/使用中/异常的表达」已于 2026-09-21 由 D3 `#7` resolve，产物 `docs/spec/03-order-list.md`；**「黑名单与低信用分的降级体验」已于 2026-09-22 由 D4 `#8` resolve**，产物 `docs/spec/04-blacklist-and-degradation.md`；「C 端文案体系」已是 live ticket T1 `#13`。）
...
**Frontier（可立即开）**：D5、D6、D7、P1、T1、T2 —— R1/R2/R3（2026-09-20）、**D1**（2026-09-20）、**D2**（2026-09-21）、**D3**（2026-09-21）、**D4**（2026-09-22）关闭后，**全部子票解锁**（含此前被 D3 阻塞的 T1）。
...
**D4 已 resolve（`#8`，2026-09-22）** —— 产物 **`docs/spec/04-blacklist-and-degradation.md`** + `CONTEXT.md` 三术语；就地补正 `01` §5 / `03` §12，更正 R1 findings（`research/r1-api-mapping` commit `19c8bea`）。三条登记项见 `04` §12（含一条**上线前置项**：小程序后台须配客服）。
